...
class XSpacesScraper:
            ...
            try:
                ...
                for sid in sorted(candidate_ids):
                    if not re.fullmatch(r"[A-Za-z0-9]{12,20}", sid):
                        continue
                    info = self._get_audio_space_info(sid, guest_token)
                    if not info:
                        continue
    ...
    def check_spaces(self, user_ids: list[str], username_map: dict[str, str] | None = None) -> list[dict]:
        ...
        for uid in user_ids:
            ...
            if not username:
                continue
            found = self._live_spaces_for_username(username, guest_token)
            for item in found:
        ...
        for uid in user_ids:
            candidate_ids = self._collect_space_ids_from_user_tweets(str(uid), guest_token)
