...
from infrastructure.discord.playback.channel_status_service import restore_channel_status, set_channel_status
from infrastructure.discord.playback.playback_summary_service import build_end_notice, session_details_snapshot
from infrastructure.discord.playback.text_notification_service import notify_text_channel
from infrastructure.runtime.text_utils import format_elapsed
...
class DiscordVoiceGateway:
    ...
    async def play(
        ...
        async with lock:
            ...
            raw_source = discord.FFmpegPCMAudio(
                stream_url,
                before_options=f"{before_options} -nostdin -hide_banner",
                options="-vn -sn -dn -map_metadata -1",
            )
    ...
    async def _set_channel_status(self, session, channel, text: str):
        await set_channel_status(session, channel, text)

    async def _restore_channel_status(self, session, guild):
        await restore_channel_status(session, guild)
    ...
    def _session_details_snapshot(self, session) -> dict:
        return session_details_snapshot(session)

    def _build_end_notice(self, reason: str, details: dict) -> str:
        return build_end_notice(reason, details)

    async def _notify_text_channel(self, guild: discord.Guild, message: str, preferred_channel_id: int = 0) -> None:
        await notify_text_channel(guild, self.sessions, message, preferred_channel_id=preferred_channel_id)
