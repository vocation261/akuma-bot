...
from infrastructure.runtime.text_utils import (
...
from infrastructure.transcription import download_space_audio, transcribe_audio_batch
from infrastructure.transcription.downloader import fetch_space_metadata
from infrastructure.transcription.transcriber import split_audio_max_1h
from infrastructure.security import InputValidator, ValidationError

logger = logging.getLogger("akuma_bot")


def _transcript_ended_space_check(metadata: dict[str, object]) -> tuple[bool, str]:
    status_key = str(metadata.get("status_key") or "").strip().lower()
    if bool(metadata.get("is_live")) or status_key in {"live", "running", "scheduled"}:
        return False, "❌ /transcript only works for ended Spaces."
    if status_key != "ended":
        return False, "❌ Could not confirm that the Space has ended yet. Try again once it is clearly marked as finished."
    return True, ""
...
async def _upsert_panel_in_invocation_message(
    interaction: discord.Interaction,
    panel_gateway,
    *,
    note: str = "",
    ...
    return await panel_gateway.upsert(
        interaction.guild,
        interaction.channel,
        note=note,
...
def register_commands(tree: app_commands.CommandTree, deps) -> None:
    ...
    async def transcript_command(interaction: discord.Interaction, url: str):
        ...
        try:
            ...
            can_transcribe, transcript_error = _transcript_ended_space_check(metadata)
            if not can_transcribe:
                status_msg = await _safe_message_update(status_msg, transcript_error, channel)
                return
    ...
    async def dash_command(interaction: discord.Interaction):
        ...
        if not interaction.guild:
            await interaction.edit_original_response(content="Use this command in a guild.")
            return
        try:
            await _upsert_panel_in_invocation_message(interaction, panel_gateway)
